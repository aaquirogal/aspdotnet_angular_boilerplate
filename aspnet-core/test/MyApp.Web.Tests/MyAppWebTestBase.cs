...
{
    public abstract class MyAppWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static MyAppWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(MyAppWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<MyAppDbContext> action)
        {
            using (var context = IocManager.Resolve<MyAppDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<MyAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<MyAppDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<MyAppDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<MyAppDbContext, Task<T>> func)
        {
            T result;
